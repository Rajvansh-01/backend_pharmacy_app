...
var checkUserAuth = async (req, res, next) => {
  //If the user has to access protected routes, then he must pass the token which was generated while logging in
  let token;
  const { authorization } = req.headers; //headers consists of all headers that comes from the client
  if (authorization && authorization.startsWith("Bearer")) {
    //Check if any value is there in authorization and check if authorization starts with a Bearer
    try {
      ...
      //Get User from Token(based on userID)
